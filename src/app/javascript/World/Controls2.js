...
export default class Controls1 extends EventEmitter
{
    ...
    setTouch()
    {
        ...
        // Create the shoot button element
        this.touch.shoot.$element = document.createElement('div');
        this.touch.shoot.$element.style.position = 'fixed';
        this.touch.shoot.$element.style.right = '100px';
        this.touch.shoot.$element.style.bottom = '100px';
        this.touch.shoot.$element.style.width = '20px';  // Adjust size as needed
        this.touch.shoot.$element.style.height = '20px';
        this.touch.shoot.$element.style.backgroundImage = `url(${mobileCross})`;
        this.touch.shoot.$element.style.backgroundSize = 'cover';
        this.touch.shoot.$element.style.zIndex = '10';  // Ensure it's clickable
        document.body.appendChild(this.touch.shoot.$element);
        
        // Define event listeners for shooting
        this.touch.shoot.events = {};

        this.touch.shoot.events.touchstart = (_event) => {
            _event.preventDefault();
            this.trigger('shootStart');  // Custom event for starting the shoot
            this.actions.shoot = true;  // Set an action flag for shooting

            // Implement the delay and shooting logic
            setTimeout(() => {
                if (this.actions.shoot) {
                    this.trigger('shoot');  // Emit an event or call your shooting function
                    console.log('Shooting!');
                }
                this.actions.shoot = false;  // Reset shoot action
            }, 5000);  // Delay for 5 seconds before firing
        };
        ...
        this.touch.shoot.events.touchend = (_event) => {
            _event.preventDefault();
            if (this.actions.shoot) {
                console.log('Shooting cancelled.');
            }
            this.actions.shoot = false;  // Cancel the shooting action
        };
        
        this.touch.shoot.$element.addEventListener('touchend', this.touch.shoot.events.touchend);        
